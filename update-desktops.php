...
if (isset($_GET['Asset_No'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE Desktops SET Asset_No = ? , Make = ?, Model = ?, RAM = ?, CPU = ?, MAC_Address = ? , Serial_Number = ?, Product_Number = ?,  Location = ?, Assigned = ? WHERE Asset_No = ?');
    ...
    $stmt = $pdo->prepare('SELECT * FROM Desktops WHERE Asset_No = ?');
...
<?=template_header('Update Desktops')?>
